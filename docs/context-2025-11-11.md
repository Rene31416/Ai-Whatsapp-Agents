...
- Controller + Lambda handler (`src/controller/appointments.controller.ts`, `src/app/lambda-handlers/clinic.ts`) expose:
  - `POST /appointments`
...
- CDK wires the shared `ClinicLambda` to both `/appointments` and `/clinic` routes, granting table permissions and injecting env vars.
